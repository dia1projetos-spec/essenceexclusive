...
async function loadDataFromFirebase() {
    ...
    if (window.FirebaseHelpers) {
        try {
            await window.FirebaseHelpers.loadProducts();
            await window.FirebaseHelpers.loadCategories();
            await window.FirebaseHelpers.loadSlides();
            console.log('✅ Data loaded from Firebase');
        } catch (error) {
            console.warn('⚠️ Using local cache');
        }
    }
}
